package curiouscreature.kotlin.math
...
class Quat {
    ...
    constructor(euler: Float3) {
        setEuler(radians(euler.y), radians(euler.x), radians(euler.z))
    }
    /** Set the rotation using axis angle notation
     * @param axis The axis around which to rotate
     * @param angle The magnitude of the rotation in Radians */
    fun setRotation(axis: Float3, angle: Float) {
        val d = length(axis)
    ...
    fun getEulerZYX(yawZ_pitchY_rollX: FloatArray) {
        ...
        val sarg = -2f * (x * z - w * y)
        yawZ_pitchY_rollX[1] = if (sarg <= -1f) (-0.5f * kotlin.math.PI).toFloat() else if (sarg >= 1f) (0.5f * kotlin.math.PI).toFloat() else asin(sarg)
    ...
    operator fun times(v: Float3) = Quat(
            w * v.x + y * v.z - z * v.y,
            w * v.y + z * v.x - x * v.z,
            w * v.z + x * v.y - y * v.x,
            -x * v.x - y * v.y - z * v.z)
    ...
    fun getAxis(): Float3 {
        val sSquared = 1f - w * w
        if (sSquared < 10f * EPSILON) //Check for divide by zero
            return Float3(1f, 0f, 0f)  // Arbitrary
    ...
    private fun assert(b: Boolean) {
        if (!b) throw Error("Assertion error")
    }

    infix fun rotate(v: Float3): Float3 {
        val q = this * v
        q *= this.inverse()
...
/*
fun shortestArcQuat(v0: Float3, v1: Float3): Quat {
    val c = v0 cross v1
    val d = v0 dot v1
...
fun shortestArcQuatNormalize2(v0: Float3, v1: Float3): Quat {
    v0.normalize()
    v1.normalize()
